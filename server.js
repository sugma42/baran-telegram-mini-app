...
function validateTelegramInitData(initData) {
    ...
    try {
        ...
        if (!receivedHash) {
            console.warn("⚠️ В initData нет hash");
            return null;
        }
        ...
        if (calculatedHash.length !== receivedHash.length) {
            console.warn("⚠️ Длина хеша не совпадает");
            return null;
        }
        ...
        if (!crypto.timingSafeEqual(
            ...
            console.warn("⚠️ Хеши не совпадают");
            return null;
        }

        const userData = params.get("user");
        if (!userData) {
            console.warn("⚠️ В initData нет user");
            return null;
        }
...
function adminOnly(req, res, next) {
    ...
    const currentId = String(req.telegramUser?.id || "");
...
for (const a of symbols) {
    for (const b of symbols) {
        for (const c of symbols) {
            let multiplier = 0;
            if (a === b && b === c) {
                if (a === "7️⃣") multiplier = 4;
                else if (a === "🍋") multiplier = 2.5;
                else if (a === "🍒") multiplier = 2;
                else multiplier = 0;
            } else {
                const counts = {};
                [a, b, c].forEach(symbol => counts[symbol] = (counts[symbol] || 0) + 1);
                const maxCount = Math.max(...Object.values(counts));
                if (maxCount === 2) {
                    if (counts["7️⃣"] === 2) multiplier = 1.4;
                    else if (counts["🍋"] === 2) multiplier = 1.15;
                    else if (counts["🍒"] === 2) multiplier = 1.1;
                } else {
                    if (a === "🍋" && b === "🍒" && c === "🍋") multiplier = 0.5;
                    else if (a === "🍒" && b === "🍋" && c === "🍒") multiplier = 0.5;
                }
            }
            let weight = 10;
            if (multiplier >= 4) weight = 0.4;
            else if (multiplier >= 2) weight = 1.5;
            else if (multiplier >= 1.4) weight = 3;
            else if (multiplier >= 1) weight = 5;
            else if (multiplier > 0) weight = 7;
            combinations.push({ combo: [a, b, c], multiplier, weight });
        }
    }
}
...
app.post("/api/spin", authenticate, (req, res) => {
    try {
        ...
        const grossWin = Math.floor(bet * result.multiplier);
        let playerWin = grossWin;
        let adminCommission = 0;
        if (grossWin > 0) {
            adminCommission = Math.floor(grossWin * 0.10);
            playerWin = grossWin - adminCommission;
        }

        changeBalance(user.id, -bet);
        if (playerWin > 0) changeBalance(user.id, playerWin);
        if (adminCommission > 0) changeAdminBalance(adminCommission);
        ...
        addTransaction({
            userId: user.id,
            type: "spin",
            amount: playerWin - bet,
            description: `Ставка ${bet}, результат ${result.multiplier}x`
        });

        const updatedUser = getUserByTelegramId(user.tg_id);
        res.json({
            success: true,
            result: {
                combo: result.combo,
...
app.get("/api/history", authenticate, (req, res) => {
    try {
        const user = getOrCreateUser(req.telegramUser);
        const history = getUserSpins(user.id, 20);
        res.json({
            success: true,
            history: history.map(spin => ({
                id: spin.id,
                bet: spin.bet,
                combo: [spin.symbol1, spin.symbol2, spin.symbol3],
...
app.post("/api/admin/demo-withdraw", authenticate, adminOnly, (req, res) => {
    try {
        ...
        const stats = getAdminStats();
        if (stats.adminBalance < amount) {
            return res.status(400).json({
                success: false,
                error: "Недостаточно средств"
            });
        }
        changeAdminBalance(-amount);
        res.json({
            success: true,
            adminBalance: getAdminStats().adminBalance
        });
    } catch (error) {
        console.error(error);
        res.status(500).json({
